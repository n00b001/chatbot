...
def copy_ignore(path, content):
    ...
    print('\n'.join(
        ['{}Copying:{} {}/{}'.format(colorama.Fore.GREEN, colorama.Fore.RESET, path, file) for file in content if
         file not in blacklist]))
    return blacklist


def prepare_deploy():
    # Find all available checkpoints
    checkpoints = [file[:-6] for file in os.listdir(hparams['out_dir']) if
                   os.path.isfile(hparams['out_dir'] + file) and file[-6:] == '.index']
    ...
    try:
        ...
        if e.errno == errno.EEXIST:
            print('{}Deployment folder already exists, (re)move it to continue{}'.format(colorama.Fore.RED,
                                                                                         colorama.Fore.RESET))
            sys.exit()
        else:
            raise
    ...
    for index, checkpoint in enumerate(checkpoints):
        print("{}{}.{} {}{}".format(colorama.Fore.GREEN, index + 1, colorama.Fore.RESET,
                                    '*' if checkpoint in default_checkpoint else '', checkpoint))
    ...
    paths = ['core', 'nmt', 'setup/settings.py', 'setup/answers_replace.txt', 'setup/answers_subsentence_score.txt',
             'utils/run_tensorboard.py', 'inference.py', 'requirements.txt', hparams['out_dir'] + 'hparams']
    # Append source vocab
    ...
    paths.extend([hparams['out_dir'] + file for file in os.listdir(hparams['out_dir']) if
                  file.startswith(checkpoints[choice - 1])])
    # Copy all files
    ...
    while True:
        try:
            with open('_deployment/' + hparams['out_dir'] + 'checkpoint', 'w', encoding='utf-8',
                      newline='') as checkpoint_file:
